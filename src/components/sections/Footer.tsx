    ...
    <footer className="relative bg-espresso text-cream overflow-hidden">
      ...
      <div className="container-wide py-14 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
        <div className="col-span-2 lg:col-span-2">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <span className="flex flex-col leading-none">
              <span className="font-heading font-extrabold text-cream text-lg">{SITE.name}</span>
              <span className="font-heading font-semibold text-gold text-[0.65rem] uppercase tracking-[0.18em]">{SITE.tagline}</span>
            </span>
          </div>
        ...
        <div>
          ...
          <ul className="space-y-2.5">
            {LOCATIONS.slice(0, 5).map((l) => (<li key={l.slug}><Link href={`/locations/${l.slug}`} className="text-cream/70 hover:text-gold text-sm transition-colors">{l.name}</Link></li>))}
        ...
        <div>
          ...
          <ul className="space-y-3.5 text-sm">
            ...
            <li className="flex items-start gap-2.5 text-cream/80"><MapPin className="h-4 w-4 mt-0.5 text-gold flex-shrink-0" /><span>{SITE.address}</span></li>
            <li className="flex items-start gap-2.5 text-cream/80"><Clock className="h-4 w-4 mt-0.5 text-gold flex-shrink-0" /><span>Mon–Fri 8am–5pm (MST)<br /><span className="text-xs text-cream/60">Same-day certificates</span></span></li>
          </ul>
        </div>
      </div>
